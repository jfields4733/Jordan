`timescale 1ns / 1ps
//this module and all modules it is based upon are combinatory .. no clocks
//this means everything is running in parallel constantly .. no flip flops (except the d2to4decoder and d3to8decoder which are using reg ... really latch)
//this circuit is designed to be used by another circuit that is using clock to drive Annode as a four bit counter
//The inputs to this circuit are data to be displayed (hexToDisplay) and iDP, input Decimal Place
//   these inputs could come from another circuit that has the anoode driving clock in it
//It is designed for debugging purposes so hexToDisplay and iDP are driven manually by switches, taking for switch inputs away from the fourth 7seg display
//The hex value displayed on the third and fourth 7 seg display are the same .. see hexToDisplay[11:8] repeated below
//To use all eight 7seg displays, change hexToDisplay to [31:0], change iDP to [3:0], change Annode to [3:0], change the wires to [3:0]
//   add four d7segDisplay instantiations  
//   switch from d2to4decoder to d3to8decoder .. to the 3 to 8 decoder

module d4x7segDisplay(
	input [15:0] hexToDisplay, //uses 12 bits because have 16 switches and need 4 of them to manually control the anodes and decimal place
	...
	d7segDisplay u3(hexToDisplay[15:12],iDecimalPlace[3], iAnnode[3], a,b,c,d,e,f,g,oDP); 
